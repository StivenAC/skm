...
const checklistsSvc = require('../checklists/checklists.service');
...
const SERVICE_TYPE_CHECKLISTS = {
  MECHANICAL: 'tpl-motocross-std',
  BASIC_PREPARATION: 'tpl-basic-preparation',
};
...
const create = async (data, req) => {
  ...
  const templateId = SERVICE_TYPE_CHECKLISTS[service.serviceType || 'MECHANICAL'];
  if (templateId) {
    await checklistsSvc.create(service.id, { templateId }, req.user.id);
  }
...
const update = async (id, data, req) => {
  ...
  const allowedFields = [
    'serviceType', 'status', 'priority', 'slaHours', 'estimatedHours', 'estimatedCost', 
